...
function addToCode(digit) { // פונקציה להוספת נקישה
    audioBeep.play();
    color = document.getElementById("button" + digit).style.backgroundColor = "blue"; // צביעה של הלחצן
...
function login() {
    ...
    const settings = {
        url: "https://final-project-noad-b-4faa01.appdrag.site/api/auth/login",
        data: {
            username: username,
            Pass_Code: code
        },
...
async function updateUserDB(btn) {
    ...
    if (x == true) {
        ...
        if (verify == false) { // אם הבדיקה יצא שקר אז כנס לפה 
            ...
            var settings = {
                url: "https://final-project-noad-b-4faa01.appdrag.site/api/auth/updateUser",
                data: {
...
async function addU() { // זה בעצם טופס
    ...
    if (verify === false) {
        ...
        View.protection(_username,_Role,text);
        var settings = {
            url: "https://final-project-noad-b-4faa01.appdrag.site/api/auth/AddUser",
            data: {
                username: _username,
                FirstName: _FirstName,
                Pass_Code: _Pass_Code,
...
async function verifyUsername(username) { // בודק אם קיים כבר שם משתמש כזה
    var verify = "";
    var settings = {
        url: "https://final-project-noad-b-4faa01.appdrag.site/api/auth/verifyUsername",
        data: {
            username: username
        },
        method: "POST"
...
function deleteU(btn) {
    ...
    if (x == true) {
        ...
        const settings = {
            url: "https://final-project-noad-b-4faa01.appdrag.site/api/auth/DeleteUser",
            data: {
                username: _username
            },
            method: "DELETE"
